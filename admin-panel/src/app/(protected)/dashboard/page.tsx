...
export default function Dashboard() {
    ...
    const downloadDomains = useCallback(() => {
        if (!stats?.recent_results || stats.recent_results.length === 0) {
            alert('No domains to download');
            return;
        }

        // Extract unique domains
        const uniqueDomains = Array.from(new Set(stats.recent_results.map(r => r.domain)));

        // Create CSV content with one domain per line
    ...
    const sendToTelegram = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('/api/telegram/send-domains', {
                method: 'POST',
            ...
            setTelegramMessage(`❌ Error: ${err instanceof Error ? err.message : 'Unknown error'}`);
        } finally {
    ...
    return (
        <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            {usage && (
                <div>
                    <h2 className="text-2xl font-bold font-mono text-transparent bg-clip-text bg-gradient-to-r from-lime-400 to-cyan-400 mb-6" style={{ textShadow: '0 0 20px rgba(0,255,100,0.5)' }}>▶ SUPABASE_INFRASTRUCTURE</h2>
                    <UsageStats
                        egress={usage.egress}
                        database_size={usage.database_size}
                        monthly_active_users={usage.monthly_active_users}
                        storage_size={usage.storage_size}
                    />
                </div>
            )}
            ...
            <div>
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold font-mono text-transparent bg-clip-text bg-gradient-to-r from-lime-400 to-cyan-400" style={{ textShadow: '0 0 20px rgba(0,255,100,0.5)' }}>▶ DATA_STREAM LIVE</h2>
                    <div className="flex gap-3">
                        <button
                            onClick={sendToTelegram}
                            disabled={telegramLoading}
                            className="px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white rounded border-2 border-blue-400 font-mono font-bold hover:from-blue-400 hover:to-cyan-400 transition-all text-sm uppercase disabled:opacity-50" style={{ boxShadow: '0 0 15px rgba(0,150,255,0.6)' }}
                        >
                            {telegramLoading ? '⏳ SENDING...' : '📤 SEND TO TELEGRAM'}
                        </button>
                        <button
                            onClick={downloadDomains}
                            className="px-4 py-2 bg-gradient-to-r from-purple-500 to-orange-500 text-white rounded border-2 border-orange-400 font-mono font-bold hover:from-orange-400 hover:to-purple-400 transition-all text-sm uppercase" style={{ boxShadow: '0 0 15px rgba(255,165,0,0.6)' }}
                        >
                            📥 DOWNLOAD DOMAINS
                        </button>
                        <button
                            onClick={fetchStats}
                            className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-green-500 text-black rounded border-2 border-green-400 font-mono font-bold hover:from-green-400 hover:to-cyan-400 transition-all text-sm uppercase" style={{ boxShadow: '0 0 15px rgba(0,255,100,0.6)' }}
                        >
                            ⚡ REFRESH
                        </button>
                    </div>
                ...
                {telegramMessage && (
                    <div className={`mb-4 p-3 rounded border-2 font-mono text-sm ${telegramMessage.includes('✅') ? 'bg-green-900 border-green-500 text-green-300' : 'bg-red-900 border-red-500 text-red-300'}`}>
                        {telegramMessage}
                    </div>
                )}
                <DomainTable results={stats?.recent_results || []} />
            </div>

            {/* System Status */}
            <div className="bg-gray-950 border-2 border-green-500 rounded p-6 mt-8" style={{ boxShadow: '0 0 20px rgba(0,255,100,0.3), inset 0 0 10px rgba(0,255,100,0.1)' }}>
                <div className="flex items-center gap-3 mb-2">
                    <span className="inline-block w-3 h-3 rounded-full bg-lime-400 animate-pulse" style={{ boxShadow: '0 0 10px rgba(0,255,100,0.8)' }}></span>
                    <h3 className="text-lg font-mono font-bold text-lime-400" style={{ textShadow: '0 0 15px rgba(0,255,100,0.7)' }}>SYSTEM_STATUS: OPERATIONAL</h3>
                </div>
                <p className="text-green-300 text-sm font-mono" style={{ textShadow: '0 0 10px rgba(0,255,100,0.5)' }}>$ DuckDuckGo scraper is running in real-time monitoring mode. Data refreshes automatically every 30 seconds. All results are persisted in Supabase infrastructure. Status: <span className="text-lime-400 font-bold">ACTIVE</span></p>
            </div>
        </div>
    );
}
